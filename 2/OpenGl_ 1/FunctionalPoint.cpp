...
Shifts FunctionalPoint::
shiftsSUM(Shifts shifts, const Shifts& SHIFTS_EXTERN_INTERACT)
{
...
sumShiftObjectCoord(CoordsOBJ& оbjectCoordinats, const Shifts SHIFTS )
{
...
void FunctionalPoint::calcForce(Point& point, const size_t F, const AngleDeviat ANGLE)
{
...
void FunctionalPoint::mainCalcWeakForce(Point& point_LEFT, Point& point_REGHT)
{
	double proportia;
		...
		(point_LEFT.shiftsInertia.shiftPlaneX <= 0 && point_REGHT.shiftsInertia.shiftPlaneX <= 0))
	{
			...
			point_LEFT.weight >= point_REGHT.weight)
		{
			...
			//double proportia = (double)point_LEFT.weight / (double)point_REGHT.weight;
			...
			point_LEFT.weight <= point_REGHT.weight)
		{
			...
			//double proportia = (double)point_REGHT.weight / (double)point_LEFT.weight;
