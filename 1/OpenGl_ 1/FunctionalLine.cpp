...
bool FunctionalLine::inspectorCoordsd(const Point& point, const Point& point_LEFT,
	const size_t DEVIATION)
{
	{
		int POS_X_LEFT, POS_Y_LEFT;
		//может тут поколдовать
		...
		//node.coordsObj.POS_X == myCoords.POS_X &&
	}
}
bool FunctionalLine::inspectorCoords(const Point& point, const Point& point_LEFT,
	const Point& point_REGHT, const size_t DEVIATION, const size_t F)
{
...
void FunctionalLine::
verificationExternalInteractions_RIGHT(std::vector<Line>& vecShapes,
	const size_t NUMBER_SPACE,
	const size_t NUMBER_AREA,
	const size_t NUMBER_LINE,
				...
				/* if (F < 800)
				 ...
				 for (; j < SIZE_LINE; ++j)
				 {
					 ...
					 if (inspectorCoordsd(vecShapes[NUMBER_LINE].
						...
						/* if (inspectorCoords(vecShapes[i].vecPoints[j], vecShapes[NUMBER_LINE].
							 vecPoints[NUMBER_POINT],
							 vecShapes[NUMBER_LINE].
							 vecPoints[NUMBER_POINT + index], 1000, F))*/
					 {
						 FunctionalPoint::mainCalcWeakForce(vecShapes[NUMBER_LINE].
							 vecPoints[NUMBER_POINT], vecShapes[i].vecPoints[j]);
...
void FunctionalLine::mainCalcWeakInteraction(std::vector<Line>& vecLine, Line& line,
	const size_t NUMBER_SPACE,
	const size_t NUMBER_AREA,
	const size_t NUMBER_LINE)
{
	const int SIZE = line.vecPoints.size();
		...
		for (size_t i(0); i < SIZE; ++i)
		{
			if (line.vecPoints[i].coordsObj.POS_Y > LAND_MARK)
			{
				...
				{
					verificationExternalInteractions_RIGHT(vecLine, NUMBER_SPACE,
						NUMBER_AREA, NUMBER_LINE, i);
				}
				// сделать проверку на внешнее взаимодействие
				//FunctionalPoint::sumShiftObjectCoord(line.vecPoints[i].coordsObj,
				//	line.vecPoints[i].inertia);
				FunctionalPoint::sumShiftObjectCoord(line.vecPoints[i].coordsObj,
